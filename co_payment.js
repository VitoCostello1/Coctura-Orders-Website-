...
window.addEventListener("load", function() {
   ...
   // Update formData replacing the "+" with space
   formData = formData.replace(/\+/g, " ");
   // Update formData again replacing any URI components with regular characters
   formData = decodeURIComponent(formData);
   // split the formData string anywhere that there is an "=" or a "&"
   let formFields = formData.split(/[&=]/g);

   // Write the field values from the array to the payment form 
...
function runSubmit() {
   // this function runs all separate validation based functions
   validateName();
   validateCredit();
   validateNumber();
   validateMonth();
